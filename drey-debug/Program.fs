...
and
    ...
    static member Blank = 
        { mb = Unchecked.defaultof<_>; program = Unchecked.defaultof<_>; disassGrid = null; tabs = null; jsonArea = null 
          objectsNode = null; fibersNode = null; gotoInstruction = Unchecked.defaultof<_>}
    member this.disassemble(newProgram:Program) =
        ...
        while index < this.program.program.Length do
            let b = readByte()
            let (opcode,extended) = this.program.opcodes.[b]
...
let parseProgramResponse (json:JObject) =
    ...
    let opcodes =
        ...
        |> Seq.map(fun x -> 
                     ...
                     let extended = x.Value.["extended"].Value<int>() = 1
...
let handleMessage  (messageType:MessageType) (json:string seq) (ui:UI) =
    ...
    | Debug ->
        ...
        if jo.["type"].Value<string>() <> "get-program" then
            ...
            | "announce" ->
                f.BeginInvoke(new MethodInvoker( fun _ -> 
                    ...
                    let existing = 
                        ...
                        |> Map.ofSeq
////                        
                    for k, f in newFibers do
                        if existing.ContainsKey k then
                            ()
                        else    
                            let fibNode = TreeNode(k,  Tag = f)
                            ui.fibersNode.Nodes.Add(fibNode) |> ignore
                            f.["exec_contexts"]
                            |> Seq.iteri(fun i ec ->
                                let ecNode = TreeNode(sprintf "ec %i" i, Tag = ec)
                                fibNode.Nodes.Add(ecNode) |> ignore
                                ec.["scopes"]
                                |> Seq.iteri(fun i scope ->
                                    let scopeNode = TreeNode(sprintf "scope %i" i, Tag = scope)
                                    ecNode.Nodes.Add(scopeNode) |> ignore )


                                )

                            //existing.[k] <- kvp.Value
...
let createUi (inbox:MailboxProcessor<DebugMessageType>) =
    ...
    let fibersNode = new TreeNode("Fibers")
    ...
    { disassGrid = disassemblyGrid
      ...
      fibersNode = fibersNode
      program = Program.Blank }



let context = new Context()
let client  = dealer context
